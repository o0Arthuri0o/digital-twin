...
Важно: диапазон `S5` уже ограничен в UI и в логике hand tracking (35 закрыт, 90 открыт), поэтому на стороне Arduino всё равно рекомендуется повторно делать `clamp` как защиту.
...
- пальцы захвата расходятся симметрично через `rotation.z`
...
- S5 (захват): pinch-дистанция `thumbTip <-> indexTip` в диапазон 35..90.
